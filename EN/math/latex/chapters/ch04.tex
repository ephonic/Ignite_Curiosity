...
The distance is $5$. We did just one thing: turned coordinate differences into two legs and called on Pythagoras. Replace the specific values with symbols, and the same reasoning works for any two points.
...
Let the triangle's vertices be $A(x_{1},y_{1})$, $B(x_{2},y_{2})$, and $C(x_{3},y_{3})$. We deliberately avoid specific numbers because the theorem concerns \textbf{all} triangles; the coordinate variables represent arbitrary choices.
