...
Let $a_1,a_2,a_3,\cdots$ be a sequence and $L$ a number. If, for every positive number $d$, however small, there is a position $N$ such that \textbf{all} terms after position $N$ lie less than $d$ from $L$, so that
\[
|a_n-L|<d \quad (n>N),
\]
...
\]
Since $|r|<1$, the absolute value of $r^n$ shrinks as $n$ increases. For example, with $r=0.1$, $r^n$ runs through $0.1,0.01,0.001,\cdots$. Given any positive $d$, sufficiently large $n$ makes $|r^n|$ small enough that $\left|\dfrac{a}{1-r}\right|\cdot|r^n|$ is less than $d$. Thus the distance from $S_n$ to $\dfrac{a}{1-r}$ becomes arbitrarily small, so by definition
\[
\lim_{n\to\infty}S_n=\frac{a}{1-r}.
\]
...
\begin{sikao}
  ...
  \item Prove that a sequence cannot converge to two different numbers $L$ and $M$. \textbf{Hint}: let the distance between $L$ and $M$ be $h>0$. Choose $d=\dfrac{h}{2}$ and draw corridors centred at $L$ and $M$. They do not overlap. What contradiction arises if the sequence must eventually stay in both?
\end{sikao}
