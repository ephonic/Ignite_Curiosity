...
\]
For $N+2$, the number $N$ contains a factor $2$, since the factorial includes $2$, so $N+2$ is divisible by $2$ and composite. Similarly, for $N+3$, the number $N$ contains a factor $3$, so $N+3$ is divisible by $3$ and composite. In general, for every $k$ from $2$ to $n+1$, the number $N$ contains $k$ as a factor. Thus $N+k$ is divisible by $k$, and $N+k$ is clearly larger than $k$, hence composite. All $n$ consecutive integers from $N+2$ to $N+(n+1)$ are composite.
\end{proof}
