...
Imagine the polyhedron as a hollow rubber balloon. Remove one face and flatten the rest onto a table, producing a connected graph of vertices and edges. It divides the plane into regions, one fewer than the original face count, since the removed face becomes the unbounded exterior. We need to prove $V-E+F_{\text{regions}}=1$ for the flattened graph. Perform two demolition operations. If a cycle remains, delete one of its edges: $E$ decreases by $1$, and the regions on its two sides merge, decreasing $F_{\text{regions}}$ by $1$, so $V-E+F_{\text{regions}}$ is unchanged. Once no cycles remain, the graph is a tree. Remove an endpoint and its incident edge: both $V$ and $E$ decrease by $1$, again preserving $V-E+F_{\text{regions}}$. Eventually only a lone vertex remains: $V=1$, $E=0$, $F_{\text{regions}}=0$, giving $1$. Since neither operation changed the value, the original planar graph also gives $1$, and restoring the polyhedron gives $2$.
\end{proof}
