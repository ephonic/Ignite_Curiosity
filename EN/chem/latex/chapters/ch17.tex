...
\node at (3.7,-0.5) {Bond energy (depth)};
\node at (2.3,0.35) {Bond length};
\node at (6.2,0.5) {Far apart: no interaction};
\node at (1.0,2.2) {Too close:};
\node at (1.0,1.75) {repulsion soars};
\end{tikzpicture}
\end{center}
...
\item \textbf{Squeezing closer}: repulsion between nuclei and between electron clouds rises sharply. Total energy turns upward, and the curve becomes almost vertical. Atoms are not soft fruit you can squeeze at will.
\end{itemize}
