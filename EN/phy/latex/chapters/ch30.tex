...
\begin{tikzpicture}[scale=1.15, >={Stealth[length=2.2mm]}]
  ...
  \node[red!60!black] at (2.0,3.35) {Isothermal heat in \(Q_1\) (\(T_1\))};
  \node[blue!60!black] at (3.6,0.35) {Isothermal heat out \(Q_2\) (\(T_2\))};
  \node[rotate=-32] at (4.35,1.45) {\small Adiabatic expansion};
  \node[rotate=62] at (0.88,2.1) {\small Adiabatic compression};
\end{tikzpicture}
\end{center}
